 ...
 * example, and the service-integration tier of the four-layer doctrine: it
 * lives alongside the unit tests in `tests/*.test.ts` under the one
 * `vitest.config.ts` whose `globalSetup` stands up both local resources, and
 * runs in-process on every PR. It exercises the real app against a Firestore
 * emulator AND Redis, both stood up and discovered by vitest.globalSetup.ts.
 *
 * Two describes, by what each proves:
 *
 *  - "cache-aside" — STATEFUL. Seeds Firestore and inspects Redis DIRECTLY:
 ...
 *
 *  - "widget route" — a 404 still drives the full cache-aside path
 *    (miss → Firestore → not found), it just doesn't pre-seed. The same
 *    describe is what the Docker release pipeline's container test exercises
 *    against the running image, proving its Redis + Firestore wiring.
 *
...
describe('cache-aside: Firestore source of truth + Redis cache', { timeout: 30_000 }, () => {
  ...
  // (not at describe scope) so the connection opens only when the suite runs.
  let db!: Firestore;
...
describe('widget route: read path + validation', { timeout: 30_000 }, () => {
