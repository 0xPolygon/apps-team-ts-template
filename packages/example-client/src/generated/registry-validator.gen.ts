...
import { BlockMetadata, BlockNumberPathParams, BlockNumberResponse, CreateMessageRequest, ErrorResponse, EventList, HealthCheckResponse, HelloResponse, ListEventsQuery, Message, MessageList, RecentMessagesQuery, ValidationErrorResponse, Widget } from '@polygonlabs/example-schemas';
import { queryOptions } from '@tanstack/react-query';
import { z, type ZodError } from 'zod';

import { client } from './client.gen.js';
import { createMessage as createMessage2, getBlockMetadata as getBlockMetadata2, getBlockNumber as getBlockNumber2, getHealthCheck as getHealthCheck2, getHello as getHello2, getMessage as getMessage2, getWidget as getWidget2, listEvents as listEvents2, listMessages as listMessages2, type Options } from './sdk.gen.js';
...
export type ListEventsErrors = {
    400: z.output<typeof ValidationErrorResponse>;
    422: z.output<typeof ErrorResponse>;
    500: z.output<typeof ErrorResponse>;
};
...
export type ListEventsInput = Omit<ListEventsData, 'query'> & {
    query?: z.output<typeof ListEventsQuery>;
};

export const listEventsInputTransformer = async (input: Pick<ListEventsInput, 'query'>) => ({ ...input.query !== undefined ? { query: await z.encode(ListEventsQuery, input.query) } : {} });

export const listEvents = async <ThrowOnError extends boolean = false, TResponseStyle extends 'fields' | 'data' = 'fields'>(options?: Options<ListEventsInput, ThrowOnError>): WrapErrors<ListEventsResponses, ListEventsErrors, ThrowOnError, TResponseStyle> => {
    const transformed = await listEventsInputTransformer(options ?? {});
    let result;
    try {
        ...
        let typedErr;
        try {
            typedErr = await listEventsErrorTransformer(err);
        }
        catch (validationError) {
            throw new ResponseValidationError(validationError as ZodError, err);
        }
    ...
    if (typeof result === "object" && result !== null && "request" in result && "response" in result && (typeof errorBearing.error === "object" && errorBearing.error !== null)) {
        if (errorBearing.error instanceof Error) {
            ...
            try {
                errorBearing.error = await listEventsErrorTransformer(errorBearing.error);
            }
            catch (validationError) {
                errorBearing.error = new ResponseValidationError(validationError as ZodError, errorBearing.error);
            }
    ...
    return result as unknown as Awaited<WrapErrors<ListEventsResponses, ListEventsErrors, ThrowOnError, TResponseStyle>>;
};

export const listEventsQueryKey = (options?: Options<ListEventsInput>) => createQueryKey('listEvents', { ...options, ...options?.query !== undefined ? { query: z.encode(ListEventsQuery, options?.query) } : {} } as Options<ListEventsData>);

export const listEventsOptions = (options?: Options<ListEventsInput>) => queryOptions<ListEventsResponse, ListEventsError, ListEventsResponse, ReturnType<typeof listEventsQueryKey>>({ queryFn: async ({ queryKey, signal }) => {
        const { data } = await listEvents2({
            ...options,
            ...queryKey[0],
            signal,
            throwOnError: true
        } as Options<ListEventsData, true>);
        return data;
