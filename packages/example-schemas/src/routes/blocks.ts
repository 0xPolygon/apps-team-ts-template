...
export function addBlockRoutes<Prev extends OperationsManifest>(r: TypedRegistry<Prev>) {
  ...
  // Codec-on-path-param stress test: `:blockNumber` is `Int64Codec`. The wire
  // shape is a digit string; the runtime shape on both client and server is
  // `bigint`. The input schema (`BlockNumberPathParams`) is registered with
  // a refId so the @polygonlabs/zod-to-openapi-heyapi plugin can emit a
  // runtime-shaped `${Op}Input` type and an input transformer that runs
  // `z.encode(BlockNumberPathParams, value)` — callers pass `bigint` and
  // get a digit string in the URL.
  //
  // Also gated by ApiKeyAuth — exercises the registry router's auth flow.
  // Without a valid x-api-key header the request fails with 401 BEFORE the
  // path codec runs, so the response validator never sees a decoded bigint.
  r.registerPath({
